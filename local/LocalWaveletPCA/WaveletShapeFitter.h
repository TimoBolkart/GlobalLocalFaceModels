...
public:
	...
	void drawNSamplesGaussian(int iCoefficient, int iComponent, int nSamples, float* pSamples, float* pCost)
	{
		_ASSERT(m_pStdDev != NULL);

		int i;
		float sigma, rcpTwoSigma2, sample;
		C3Vectorf vstd = m_pStdDev->getCoefficient(iCoefficient);

		CGaussianSample grv((long)::timeGetTime());
		grv.setMean(0.0);
		...
		case 0:
			sigma = vstd.x;
			grv.setStandardDeviation(sigma);
			break;
		case 1:
			sigma = vstd.y;
			grv.setStandardDeviation(sigma);
		...
		for (i = 0; i < nSamples; i++)
		{
			sample = grv.next();
			pSamples[i] = sample;
			pCost[i] = sample*sample * rcpTwoSigma2 * m_weight;
		}
	}
	
	void drawNSamplesUniform(int iCoefficient, int iComponent, int nSamples, float* pSamples, float* pCost)
	{
		_ASSERT(m_pStdDev != NULL);

		int i;
		float sigma, rcpTwoSigma2, coeffStart, dcoeff, sample;
		C3Vectorf vstd = m_pStdDev->getCoefficient(iCoefficient);
		...
		rcpTwoSigma2 = 1.f / (2.f * sigma*sigma);
		coeffStart = -m_sampleRegion * sigma; 
		dcoeff = sigma * 2.0 * m_sampleRegion / (float)nSamples;

		for (i = 0; i < nSamples; i++)
		{
			sample = coeffStart + dcoeff * (0.5f + (float)i);
			pSamples[i] = sample;
			pCost[i] = sample*sample * rcpTwoSigma2 * m_weight;
		}
	}
