...
void CFaceData::saveFaceOFFIndices(const char* cstrFileName, int nVertices, float* pVertices, int nIndices, unsigned int* pIndices, int nPolygonVertices, unsigned int indexIgnoreValue)
{
	...
	std::vector<int> filteredFaces;

	const int numFaces = nIndices/nPolygonVertices;
	for(int i = 0; i < numFaces; ++i)
	{
		std::vector<int> tmp;
		for(int j = 0; j < nPolygonVertices; ++j)
		{
			const int currIndex = pIndices[nPolygonVertices*i+j];
			if(currIndex != indexIgnoreValue)
			{
				tmp.push_back(currIndex);
			}
			else
			{
				break;
		...
		if(tmp.size() == nPolygonVertices)
		{
			for(int j = 0; j < nPolygonVertices; ++j)
			{
				filteredFaces.push_back(tmp[j]);
			}
		}
	}

	FILE* pFile = fopen(cstrFileName, "w");
	if (pFile == NULL)
	{
	...
	const int numFilteredFaces = filteredFaces.size()/nPolygonVertices;

	fprintf(pFile, "OFF\n");
	fprintf(pFile, "%i %i %i\n", nVertices, numFilteredFaces, 0);
	...
	for(int i = 0; i < numFilteredFaces; ++i)
	{
		fprintf(pFile, "%i ", nPolygonVertices);
		for(int j = 0; j < nPolygonVertices; ++j)
		{
			fprintf(pFile, "%i ", filteredFaces[nPolygonVertices*i+j]);
		}
...
bool CFaceData::readOFFMeshAsPatch(char *szFilename, std::vector<SStereoPatch> &patches, float g_nInDataScale)
{
	std::vector<double> coords;
	std::vector<std::vector<int>> faceIndices;
	if(!readOFF_geometry(szFilename, coords, faceIndices))
	{
		return false;
	}
	...
	int nNodes = static_cast<int>(coords.size())/3;
	for (int nId = 0; nId < nNodes; ++nId)
	{
		...
		//get position
		patch.px = vp.x*g_nInDataScale;
		patch.py = vp.y*g_nInDataScale;
		patch.pz = vp.z*g_nInDataScale;
		...
		abutil::C3Vectord unitVector;
		unitVector.set(1.0, 0.0, 0.0);

		abutil::C3Vectord binormal = vnorm.cross(unitVector);
		binormal.normalize();

		abutil::C3Vectord tangent = binormal.cross(vnorm);
		tangent.normalize();
