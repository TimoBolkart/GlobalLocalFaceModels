...
public:
	...
	CQuaternion(): w(1.0), x(0.0), y(0.0), z(0.0)
	{
	}
	
	CQuaternion(scalar sw, scalar sx, scalar sy, scalar sz): w(sw), x(sx), y(sy), z(sz)
	{
	}
	
	CQuaternion(const C3Vector<scalar>& v): w(0.0), x(v.x), y(v.y), z(v.z)
	{
	}
	...
	CQuaternion(scalar radians, C3Vector<scalar>& axis)
	{
	...
	CQuaternion(scalar azimuth, scalar altitude)
	{
	...
	CQuaternion(scalar* pq): w(pq[0]), x(pq[1]), y(pq[2]), z(pq[3])
	{
	}

	CQuaternion& set(scalar qw, scalar qx, scalar qy, scalar qz)
	{
		w = qw;
		x = qx;
		y = qy;
	...
	C3Vector<scalar> vectorPart() { return C3Vector<scalar>(x, y, z); }
	...
	void mulGrassman(const CQuaternion& q, CQuaternion& qresult)
	{
	...
	scalar dot(const CQuaternion& q) { return w * q.w + x * q.x + y * q.y + z * q.z; }
	scalar lengthSquared() { return w*w + x*x + y*y + z*z; }
	scalar length() { return (scalar) sqrt(w*w + x*x + y*y + z*z); }
	...
	CQuaternion& slerp(CQuaternion& q1, CQuaternion& q2, scalar t)
	{
