...
bool MathHelper::alignData(const std::vector<double>& modelData, const std::vector<double>& modelLandmarks, const std::vector<bool>& modelLandmarksLoaded, std::vector<double>& data
									, const std::vector<double>& dataLandmarks, const std::vector<bool>& dataLandmarksLoaded, double& s, std::vector<double>& R, std::vector<double>& t, bool bICPRefinement, bool bScaling)
{
	double sLmk(0.0);
	...
	if(bICPRefinement)
	{
		if(!MathHelper::calcICPRefinenment(modelData, data, ICP_MAX_POINT_DISTANCE, s, R, t, bScaling))
		{
			return false;
		}
	}
															...
															, double& s, std::vector<double>& R, std::vector<double>& t, bool bScaling)
{
	...
	const size_t numAlignmentLandmarks = std::min<size_t>(8, std::min<size_t>(lmks1Loaded.size(), lmks2Loaded.size()));
...
bool MathHelper::calcICPRefinenment(const std::vector<double>& modelData, const std::vector<double>& data, const double maxIPCPointDist, double& s, std::vector<double>& R, std::vector<double>& t, bool bScaling)
{
